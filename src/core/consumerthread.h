...
public:
    ...
    {
        m_thread = std::thread(&ConsumerThread<PRODUCER_POLICY>::Loop, this);
    };
    ...
    {
        // since m_active behaves like a latch, we can check for "latched" state without locking
        if (m_active) {
            // locked only so that repeated calls do not push extra ShutdownPills
            std::unique_lock<CWaitableCriticalSection> l(m_cs_shutdown, std::try_to_lock);
            if (!l.owns_lock())
                return;

            // need to check again once this lock is held
            if (m_active) {
                m_active = false;
...
private:
    ...
    //! protects Terminate()
    CWaitableCriticalSection m_cs_shutdown;
