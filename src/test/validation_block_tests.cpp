...
BOOST_AUTO_TEST_CASE(processnewblock_signals_ordering)
{
    ...
    m_validation_layer->Validate(std::make_shared<CBlock>(Params().GenesisBlock()), true);
    ...
    for (int i = 0; i < 10; i++) {
        threads.create_thread([this, &blocks]() {
            for (int i = 0; i < 1000; i++) {
                ...
                m_validation_layer->SubmitForValidation(block, true);
            }

            // to make sure that eventually we process the full chain - do it here
            for (auto block : blocks) {
                if (block->vtx.size() == 1) {
                    bool processed = m_validation_layer->Validate(block, true).block_valid;
                    assert(processed);
                }
            }
        });
