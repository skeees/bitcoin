...
TestingSetup::TestingSetup(const std::string& chainName) : BasicTestingSetup(chainName)
{
    ...
    m_validation_layer.reset(new ValidationLayer(Params()));
    m_validation_layer->Start();

    peerLogic.reset(new PeerLogicValidation(connman, *m_validation_layer, scheduler));
}
...
TestChain100Setup::CreateAndProcessBlock(const std::vector<CMutableTransaction>& txns, const CScript& scriptPubKey)
{
    ...
    m_validation_layer->Validate(shared_pblock, true);
